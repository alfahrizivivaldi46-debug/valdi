...
(async () => {
  ...
  async function loadCheatNetworkData(pinGame, tipePlatform) {
    ...
    const pinBase64 = btoa(String(pinGame));

if (tipePlatform === "kahoot") {
  targetEndpoint = `https://cheatnetwork.eu/services/kahoot/autoanswer#${pinBase64}`;
} else {
  targetEndpoint = `https://cheatnetwork.eu/services/quizizz/autoanswer#${pinBase64}`;
}
    try {
      console.log(
